...
(function (ConsensusState) {
    ...
    ConsensusState[ConsensusState["waitingCreated"] = 2] = "waitingCreated";
...
class DbftConsensusNode extends events_1.EventEmitter {
    ...
    updateTip(header, nextMiners, totalView) {
        ...
        if (this.m_tip) {
            this.logger.debug(`updateTip this.m_state=${this.m_state} ${totalView} ${this.m_tip.header.hash} ${header.hash}`);
        }
        else {
            this.logger.debug(`updateTip this.m_state=${this.m_state} ${totalView}`);
        }
    ...
    async newProposal(block) {
        ...
        if (this.m_state !== ConsensusState.waitingCreated) {
            this.logger.warn(`dbft conensus newProposal ${block.header.hash}  ${block.header.number} while not in waitingCreated state`);
            return error_code_1.ErrorCode.RESULT_SKIPPED;
        }
        if (!this.m_tip.header.isPreBlock(block.header)) {
    ...
    _onTimeout() {
        ...
        if (this.m_state === ConsensusState.waitingCreate) {
            this.m_state = ConsensusState.waitingCreated;
            let newContext = {
                curView: this.m_currView
            };
    ...
    async _nextTimeout() {
        ...
        let now = Date.now() / 1000;
        if (nextTime > now) {
            this.logger.debug(`_nextTimeout intervalCount=${intervalCount},totalView=${this.m_tip.totalView},contextView=${contextView},due=${due},tipnumber=${this.m_tip.header.number},timeout=${(nextTime - now) * 1000}`);
